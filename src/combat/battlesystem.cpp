...
void battlesystem::addEquipmentBuffs(Character* character, const Character* source) {
    ...
    for (const EquipmentEffect& effect : source->getquipe()->effects()) {
        if (effect.type != EquipmentEffectType::AddBuff || effect.buffId <= 0) {
            continue;
        }

        std::unique_ptr<buff> created = bufffactory::create(effect.buffId, -1);
        if (created) {
            character->getbuff().addBuff(std::move(created));
        }
    }
