...
class RewardEquipmentItem : public QGraphicsPixmapItem {
public:
    explicit RewardEquipmentItem(const QPixmap& pixmap, QGraphicsItem* parent = nullptr)
    ...
    std::function<void(RewardEquipmentItem*)> onClicked;
...
static QPixmap loadRuntimePixmap(const std::string& relativePath, const QSize& size) {
    QPixmap pixmap;
    const QString relative = QString::fromStdString(relativePath);
    const QString runtimePath = QDir(QCoreApplication::applicationDirPath()).filePath(relative);
    const QString sourcePath = QFileInfo(QString::fromUtf8(__FILE__)).dir().filePath("../../" + relative);
    for (const QString& path : {runtimePath, sourcePath, relative}) {
        if (pixmap.load(path)) {
...
void BattleScene::playRewardEffects(int gold, const std::vector<EquipmentRewardVisual>& equipmentDrops) {
    const QPointF center = rewardChestItem ? rewardChestItem->pos() : QPointF(sceneW * 0.5, sceneH * 0.5);

    if (gold > 0) {
        auto* text = sceneObj->addText(QString("+%1").arg(gold), QFont("Arial", 30, QFont::Black));
        text->setDefaultTextColor(QColor(255, 212, 68));
        text->setZValue(zVisualEffect + 2.0);
        const QRectF bounds = text->boundingRect();
        const QPointF start(center.x() - bounds.width() * 0.5, center.y() - 96.0);
        const QPointF end(start.x(), start.y() - 72.0);
        text->setPos(start);

        auto* floatUp = new QVariantAnimation(this);
        floatUp->setDuration(980);
        floatUp->setStartValue(0.0);
        floatUp->setEndValue(1.0);
        floatUp->setEasingCurve(QEasingCurve::OutCubic);
        connect(floatUp, &QVariantAnimation::valueChanged, this, [text, start, end](const QVariant& value) {
            const double t = value.toDouble();
            text->setPos(start + (end - start) * t);
            text->setOpacity(1.0 - t);
        });
        connect(floatUp, &QVariantAnimation::finished, this, [this, text, floatUp]() {
            sceneObj->removeItem(text);
            delete text;
            floatUp->deleteLater();
        });
        floatUp->start();
    ...
    const int count = static_cast<int>(equipmentDrops.size());
    for (int i = 0; i < count; ++i) {
        const EquipmentRewardVisual drop = equipmentDrops[static_cast<size_t>(i)];
        QPixmap pixmap = loadRuntimePixmap(drop.iconPath, QSize(76, 76));
        if (pixmap.isNull()) {
            pixmap = QPixmap(76, 76);
            pixmap.fill(drop.group == EquipmentGroup::Advanced ? QColor(190, 105, 255) : QColor(90, 185, 110));
        }

        auto* item = new RewardEquipmentItem(pixmap);
        item->setZValue(zVisualEffect + 3.0);
        item->setOffset(-pixmap.width() * 0.5, -pixmap.height() * 0.5);
        item->setOpacity(0.0);
        item->setScale(0.35);
        sceneObj->addItem(item);

        const qreal spacing = 92.0;
        const qreal xOffset = (i - (count - 1) * 0.5) * spacing;
        const QPointF start = center + QPointF(0.0, -18.0);
        const QPointF end = center + QPointF(xOffset, -150.0);
        item->setPos(start);

        auto* jump = new QVariantAnimation(this);
        jump->setDuration(620);
        jump->setStartValue(0.0);
        jump->setEndValue(1.0);
        jump->setEasingCurve(QEasingCurve::OutCubic);
        connect(jump, &QVariantAnimation::valueChanged, this, [item, start, end](const QVariant& value) {
            const double t = value.toDouble();
            const QPointF base = start + (end - start) * t;
            const qreal arc = -46.0 * std::sin(pi * t);
            item->setPos(base + QPointF(0.0, arc));
            item->setOpacity(std::min(1.0, t * 1.8));
            item->setScale(0.35 + 0.65 * t);
        });
        connect(jump, &QVariantAnimation::finished, jump, &QVariantAnimation::deleteLater);
        jump->start();

        item->onClicked = [this, drop](RewardEquipmentItem* clickedItem) {
            emit rewardEquipmentClaimed(drop.group, drop.equipmentId);

            auto* fade = new QVariantAnimation(this);
            fade->setDuration(280);
            fade->setStartValue(0.0);
            fade->setEndValue(1.0);
            connect(fade, &QVariantAnimation::valueChanged, this, [clickedItem](const QVariant& value) {
                const double t = value.toDouble();
                clickedItem->setOpacity(1.0 - t);
                clickedItem->setScale(1.0 + 0.25 * t);
            });
            connect(fade, &QVariantAnimation::finished, this, [this, clickedItem, fade]() {
                sceneObj->removeItem(clickedItem);
                delete clickedItem;
                fade->deleteLater();
            });
            fade->start();
...
bool BattleScene::preparationActive() const {
    return isPreparationPhase();
}

int BattleScene::pcRosterUnitIdAtScenePos(const QPointF& scenePos) const {
    if (!isPreparationPhase()) {
        return -1;
    }
    const QList<QGraphicsItem*> itemsAtPoint = sceneObj->items(scenePos);
    for (QGraphicsItem* graphicsItem : itemsAtPoint) {
        auto* item = dynamic_cast<UnitItem*>(graphicsItem);
        if (!item || !item->unit()) {
            continue;
        }
        Unit* unit = item->unit();
        if (!isRosterUnit(unit)) {
            continue;
        }
...
void BattleScene::createObjectPropItem(Object* object) {
    if (!object || rewardChestItem) {
        return;
    }

    rewardChestItem = new PropItem(object);
    rewardChestItem->setZValue(zUnit);
    sceneObj->addItem(rewardChestItem);
    connect(rewardChestItem, &PropItem::opened,
            this, &BattleScene::handleObjectOpened);
}

void BattleScene::createObstaclePropItem(Obstacle* obstacle) {
    if (!obstacle || !sceneObj || !layout) {
        return;
    }

    Board* board = activeBoard();
    Hex h;
    if (!board || !board->posOf(obstacle, &h)) {
        return;
    }

    const QPointF hexPos = layout->toWorld(h);
    auto* item = new PropItem(obstacle);
    item->setZValue(zUnit - 0.1);
    item->setPos(hexPos);
    item->setHex(h);
    sceneObj->addItem(item);
    obstacleItems.push_back(item);
}

void BattleScene::removeObjectItem(int id) {
    if (!rewardChestItem || rewardChestItem->propId() != id) {
        return;
    }
    sceneObj->removeItem(rewardChestItem);
    delete rewardChestItem;
...
void BattleScene::handleObjectOpened(int objectId) {
    if (!rewardChestItem || rewardChestItem->propId() != objectId) {
        return;
    }
...
void BattleScene::buildScene() {
    ...
    for (Unit* unit : board->units()) {
        if (Object* object = dynamic_cast<Object*>(unit)) {
            createObjectPropItem(object);
            Hex h;
            if (rewardChestItem && board->posOf(object, &h)) {
                rewardChestItem->setHex(h);
                rewardChestItem->setPos(layout->toWorld(h));
            }
        } else if (Obstacle* obstacle = dynamic_cast<Obstacle*>(unit)) {
            createObstaclePropItem(obstacle);
...
void BattleScene::createUnitItem(Unit* unit) {
    if (!unit || dynamic_cast<Object*>(unit) || dynamic_cast<Obstacle*>(unit) ||
        unitItemMap.find(unit->id()) != unitItemMap.end()) {
        return;
    }
